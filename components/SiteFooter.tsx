...
export function SiteFooter({ lang }: { lang: Language }) {
  ...
  return (
    <footer className="mt-6 border-t border-white/12 bg-[#14243A]">
      <div className="mx-auto max-w-[1620px] px-4 py-5 sm:px-6 lg:px-10 xl:px-12 2xl:px-14">
        <div className="flex flex-col gap-3 text-sm text-white/78 sm:flex-row sm:items-center sm:justify-between">
          <p>{copy.copyright}</p>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-6">
            <Link className="transition-opacity hover:opacity-100 hover:underline" href={copy.privacyHref}>
              {copy.privacyLabel}
            </Link>
            <Link className="transition-opacity hover:opacity-100 hover:underline" href={copy.termsHref}>
              {copy.termsLabel}
            </Link>
          </div>
        </div>
